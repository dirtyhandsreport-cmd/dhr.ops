...
function scoreDHR(title, summary) {
  ...
  if (["fatal","death","killed","explosion","fire","layoff","layoffs","strike","shutdown","spill","blowout","record","historic","surge","collapse","crash","billion","merger","acquired","ban","hormuz","iran","sanction"].some(w=>t.includes(w))) return 3;
  if (["hiring","jobs","rig count","price","safety","regulation","drilling","production","earnings","wireline","completions","workforce","opec","pipeline","permit","bakken","williston","north dakota","permian","wti","brent","crude","frac"].some(w=>t.includes(w))) return 2;
  return 1;
}
